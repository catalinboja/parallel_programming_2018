...
void getEvenCount(int* values, int iStart, int iFinal, unsigned int& count) {
	for (int i = iStart; i < iFinal; i++)
		if (values[i] % 2 == 0)
		{
			//m.lock();
...
int main() {
	...
	cout << endl << "*********** TEST MIN, MEDIE SI NR PARE SECVENTIAL ******";
	
	const int N = 300000000;
	int* values = generateArray(N);
	unsigned int nrNumerePare = 0;

	cout << endl << "We have values !";
	...
	*/
	...
	/*
	2 thread-uri pentru numar elemente pare
	*/
